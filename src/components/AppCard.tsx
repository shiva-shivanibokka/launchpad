    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="truncate font-display text-[18px] font-bold leading-tight text-ink">{app.company}</div>
          {app.role && <div className="mt-1 truncate font-sans text-[15.5px] text-muted">{app.role}</div>}
        </div>
        <span className="shrink-0 rounded-md border border-white/10 px-2 py-0.5 font-mono text-[13px] text-faint opacity-60 transition group-hover:opacity-100" title="Edit">
          ✎
        </span>
      </div>
      ...
      {notePreview && (
        <div className="mt-2 truncate font-sans text-[13.5px] italic text-faint" title={app.notes}>
          “{notePreview}”
        </div>
      )}

      {overdue && (
        <div className="mt-2.5 flex items-center gap-1.5 rounded-md border border-accent-amber/40 bg-accent-amber/10 px-2.5 py-1.5 font-mono text-[13px] text-accent-amber">
          ⚠ follow up{app.nextAction ? `: ${app.nextAction}` : ''}
        </div>
      )}
      {stale && (
        <div className="mt-2.5 flex items-center gap-1.5 rounded-md border border-accent-blue/30 bg-accent-blue/10 px-2.5 py-1.5 font-mono text-[13px] text-accent-blue">
