...
export default function AppCard({
  ...
  return (
    <div
      ...
      onClick={() => onEdit(app)}
      className={`group cursor-pointer rounded-xl border bg-white/[0.03] p-3 transition hover:border-white/25 hover:bg-white/[0.055] ${border}`}
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="truncate font-display text-[15px] font-bold leading-tight text-ink">{app.company}</div>
          {app.role && <div className="mt-0.5 truncate font-sans text-[13px] text-muted">{app.role}</div>}
        </div>
        <span className="shrink-0 rounded-md border border-white/10 px-1.5 py-0.5 font-mono text-[13px] text-faint opacity-60 transition group-hover:opacity-100" title="Edit">
          ✎
        </span>
      </div>
      ...
      <div className="mt-2.5 flex flex-wrap items-center gap-2">
        ...
        {d !== null && app.status !== 'wishlist' && (
          <span className="font-mono text-[12.5px] text-faint">{d === 0 ? 'today' : `${d}d ago`}</span>
        )}
      </div>

      {notePreview && (
        <div className="mt-2 truncate font-sans text-[13px] italic text-faint" title={app.notes}>
          “{notePreview}”
        </div>
      )}

      {overdue && (
        <div className="mt-2 flex items-center gap-1.5 rounded-md border border-accent-amber/40 bg-accent-amber/10 px-2 py-1 font-mono text-[12.5px] text-accent-amber">
          ⚠ follow up{app.nextAction ? `: ${app.nextAction}` : ''}
        </div>
      )}
      {stale && (
        <div className="mt-2 flex items-center gap-1.5 rounded-md border border-accent-blue/30 bg-accent-blue/10 px-2 py-1 font-mono text-[12.5px] text-accent-blue">
      ...
      {idx >= 0 && (canBack || canFwd) && (
        <div className="mt-2.5 flex items-center justify-end gap-1.5 opacity-60 transition group-hover:opacity-100">
          <button
            ...
            className="rounded-md border border-white/12 px-2 py-0.5 font-mono text-[13px] text-muted transition hover:text-ink disabled:opacity-30"
          >
            ◀
          </button>
          <button
