  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <span className="shrink-0 rounded-md border border-white/10 px-2 py-0.5 font-mono text-[15px] text-faint opacity-60 transition group-hover:opacity-100" title="Edit">
          ✎
        </span>
      </div>
      ...
      <div className="mt-2.5 flex flex-wrap items-center gap-2">
        ...
        {d !== null && app.status !== 'wishlist' && (
          <span className="font-mono text-[14.5px] text-faint">{d === 0 ? 'today' : `${d}d ago`}</span>
        )}
      </div>
      ...
      {overdue && (
        <div className="mt-2.5 flex items-center gap-1.5 rounded-md border border-accent-amber/40 bg-accent-amber/10 px-2.5 py-1.5 font-mono text-[14px] text-accent-amber">
          ⚠ follow up{app.nextAction ? `: ${app.nextAction}` : ''}
        </div>
      )}
      {stale && (
        <div className="mt-2.5 flex items-center gap-1.5 rounded-md border border-accent-blue/30 bg-accent-blue/10 px-2.5 py-1.5 font-mono text-[14px] text-accent-blue">
