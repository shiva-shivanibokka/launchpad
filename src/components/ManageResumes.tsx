...
export default function ManageResumes({ store }: { store: Store }) {
  ...
  return (
    <Card className="p-6">
      ...
      <p className="mb-4 font-sans text-[18px] leading-relaxed text-muted">
      ...
      <div className="flex flex-col gap-2">
        {store.resumes.length === 0 && (
          <div className="rounded-xl border border-dashed border-white/10 px-3 py-4 text-center font-mono text-[14px] text-faint">
        ...
        {store.resumes.map((r) => (
          <div key={r.id} className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2">
            <input
              ...
              className="min-w-0 flex-1 bg-transparent font-sans text-[16px] font-semibold text-ink outline-none"
            />
            <input
              value={r.url || ''}
              onChange={(e) => store.updateResume(r.id, { url: e.target.value || undefined })}
              placeholder="add link…"
              className="min-w-0 flex-1 bg-transparent font-mono text-[14.5px] text-muted outline-none"
            />
            {r.url && (
              <a href={r.url} target="_blank" rel="noreferrer" className="rounded-md border border-white/10 px-2 py-1 font-mono text-[13.5px] text-accent-mint hover:bg-white/5">
                open ↗
              </a>
            )}
            <button
              onClick={() => store.deleteResume(r.id)}
              className="rounded-md border border-white/10 px-2.5 py-1 font-mono text-[14px] text-faint hover:border-accent-rose/40 hover:text-accent-rose"
