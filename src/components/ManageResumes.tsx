...
export default function ManageResumes({ store }: { store: Store }) {
  ...
  return (
    <Card className="p-5">
      ...
      <p className="mb-3 font-sans text-[17px] leading-relaxed text-muted">
      ...
      <div className="flex flex-col gap-2">
        {store.resumes.length === 0 && (
          <div className="rounded-xl border border-dashed border-white/10 px-3 py-4 text-center font-mono text-[15px] text-faint">
        ...
        {store.resumes.map((r) => (
          <div key={r.id} className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2">
            <input
              ...
              className="min-w-0 flex-1 bg-transparent font-sans text-[16.5px] font-semibold text-ink outline-none"
            />
            <input
              value={r.url || ''}
