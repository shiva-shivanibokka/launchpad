...
export default function ManageResumes({ store }: { store: Store }) {
  ...
  return (
    <Card className="p-5">
      ...
      <p className="mb-3 font-sans text-[18.5px] leading-relaxed text-muted">
      ...
      <div className="mb-4 flex flex-wrap items-end gap-2">
        <div className="min-w-[180px] flex-1">
          <span className="mb-1 block font-mono text-[16.5px] uppercase tracking-wide text-muted">Resume name</span>
          <TextInput value={name} onChange={(e) => setName(e.target.value)} placeholder="AI Engineer — v3" onKeyDown={(e) => e.key === 'Enter' && add()} />
        </div>
        <div className="min-w-[180px] flex-1">
          <span className="mb-1 block font-mono text-[16.5px] uppercase tracking-wide text-muted">Link (optional)</span>
      ...
      <div className="flex flex-col gap-2">
        {store.resumes.length === 0 && (
          <div className="rounded-xl border border-dashed border-white/10 px-3 py-4 text-center font-mono text-[16.5px] text-faint">
        ...
        {store.resumes.map((r) => (
          <div key={r.id} className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2">
            <input
              ...
              className="min-w-0 flex-1 bg-transparent font-sans text-[18px] font-semibold text-ink outline-none"
            />
            <input
              value={r.url || ''}
              onChange={(e) => store.updateResume(r.id, { url: e.target.value || undefined })}
              placeholder="add link…"
              className="min-w-0 flex-1 bg-transparent font-mono text-[16.5px] text-muted outline-none"
            />
            {r.url && (
              <a href={r.url} target="_blank" rel="noreferrer" className="rounded-md border border-white/10 px-2 py-0.5 font-mono text-[16.5px] text-accent-mint hover:bg-white/5">
                open ↗
              </a>
            )}
            <button
              onClick={() => store.deleteResume(r.id)}
              className="rounded-md border border-white/10 px-2 py-0.5 font-mono text-[16.5px] text-faint hover:border-accent-rose/40 hover:text-accent-rose"
