...
export default function ResumeAnalytics({ store }: { store: Store }) {
  ...
  return (
    <Card className="p-5">
      ...
      {!hasData ? (
        <p className="font-sans text-[18.5px] leading-relaxed text-muted">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[520px] border-collapse text-left">
            <thead>
              <tr className="font-mono text-[16px] uppercase tracking-wide text-faint">
              ...
              {rows.map((r, i) => (
                <tr key={r.resume.id} className="border-t border-white/8">
                  <td className="py-2.5 pr-3">
                    <span className="font-sans text-[18px] font-semibold text-ink">{r.resume.name}</span>
                    {i === 0 && r.applied > 0 && (
                  ...
                  <td className="px-3 font-mono text-[18px] text-subtle">{r.applied}</td>
                  <td className="px-3 font-mono text-[18px] text-subtle">{r.responded}</td>
                  <td className="px-3 font-mono text-[18px] text-subtle">{r.interviewed}</td>
                  <td className="px-3 font-mono text-[18px] text-subtle">{r.offers}</td>
