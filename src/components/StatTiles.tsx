    ...
    <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5">
      {tiles.map((m) => (
        <Card key={m.label} className="p-5">
          <div className="font-mono text-[18px] uppercase tracking-wide text-muted">{m.label}</div>
          <div className="mt-2 font-display text-[44px] font-extrabold leading-none" style={{ color: m.color }}>
            {m.value}
          </div>
          <div className="mt-1.5 font-mono text-[17px] text-faint">{m.sub}</div>
        </Card>
      ))}
    </div>
  )
