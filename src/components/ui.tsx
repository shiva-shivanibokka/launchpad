...
export function SectionTitle({ icon, title, right }: { icon: string; title: string; right?: ReactNode }) {
  return (
    <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
      <h2 className="flex items-center gap-3 font-display text-[28px] font-extrabold uppercase tracking-tight text-ink sm:text-[34px]">
...
export function Pill({ children, color = '#a78bfa' }: { children: ReactNode; color?: string }) {
  return (
    <span
      className="inline-flex items-center gap-1 rounded-full border px-3 py-1 font-mono text-[14.5px] font-bold"
...
export function Field({ label, children }: { label: string; children: ReactNode }) {
  return (
    <label className="block">
      <span className="mb-1.5 block font-mono text-[13px] uppercase tracking-wide text-muted">{label}</span>
      {children}
    </label>
  )
}
...
const inputCls =
  'w-full rounded-lg border border-white/10 bg-white/[0.04] px-3.5 py-2.5 font-sans text-[16px] text-ink outline-none transition focus:border-accent-violet/60'
...
export function Button({
  ...
  return (
    <button
      {...rest}
      className={`whitespace-nowrap rounded-lg border px-4 py-2.5 font-mono text-[15.5px] font-bold transition disabled:opacity-50 ${styles} ${rest.className || ''}`}
    >
      {children}
    </button>
  )
