// Optional persistence: commit the tracker document (applications.json) straight
// to the repo using a user-provided fine-grained PAT, stored ONLY in this
// browser's localStorage — never in the repo. Lets your applications persist and
// follow you across devices. Token needs Contents: read & write on launchpad.
// Everything works fully without a token (data just lives in this browser).

const OWNER = 'shiva-shivanibokka'
const REPO = 'launchpad'
const TOKEN_KEY = 'launchpad-gh-token'
...
export async function verifyToken(token: string): Promise<{ ok: boolean; message: string }> {
  try {
    ...
    if (r.status === 404) return { ok: false, message: 'Token can’t see the launchpad repo' }
